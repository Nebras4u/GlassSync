#!/usr/bin/env python3
"""
Digital Twin - Motion Sequence with Fixed IK
"""
...
def is_ik_successful(result):
    ...
    # In CoppeliaSim, result is a tuple: (return_code, ...)
    # Return code 1 means success
...
def move_to_position(sim, simIK, simBase, simTarget, ikEnv, ikGroup, position, target_name=""):
    ...
    try:
        ...
        if is_ik_successful(result):
            ...
            print(f"   ⚠️ IK failed with code: {result}")
            return False
...
def main():
    ...
    print("\n📊 Live Data:")
    print("-" * 100)
    print(f"{'Step':<8} {'Target (X,Y,Z,A)':<30} {'Robot Pose (X,Y,Z)':<30} {'Status'}")
    print("-" * 100)
    ...
    try:
        while sim.getSimulationState() != sim.simulation_stopped and running and step < total_steps:
            ...
            for _ in range(30):  # Allow time for movement
                sim_client.step()
                time.sleep(0.02)
            ...
            if success and distance < 5.0:
                ...
                status = f"⏳ Moving... ({distance:.1f}mm)"
            else:
                status = "⚠️ IK Issue"
            ...
            if len(joint_angles) >= 3:
                print(f"         Joints: J1={math.degrees(joint_angles[0]):5.1f}° J2={math.degrees(joint_angles[1]):5.1f}° J3={math.degrees(joint_angles[2]):5.1f}°")
        ...
        if step >= total_steps:
            ...
            print("\n📍 Final Position (updating live):")
            while running and sim.getSimulationState() != sim.simulation_stopped:
                robot_pos, robot_ori = get_robot_pose(sim, simTarget, simBase)
                joint_angles = get_joint_positions(sim, joints)
                print(f"\r   X:{robot_pos[0]*1000:6.1f}mm Y:{robot_pos[1]*1000:6.1f}mm Z:{robot_pos[2]*1000:6.1f}mm  "
