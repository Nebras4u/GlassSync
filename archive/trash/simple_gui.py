#!/usr/bin/env python3
"""
digital_twin_gui.py - Digital Twin V6.1 GUI with PLC Control
Using actual waypoints from your project (Pos1, HmPos, Pos3, Pos4)
"""
...
# ============================================================
# These are the actual waypoints used in SCL:
# Pos1, HmPos, Pos3, Pos4 are Array[1..4] of LReal
...
WAYPOINTS = {
    "pos1": {
        ...
        "angles": [930.0, 0.0, 370.0, 0.0]  # Actual values from your DB
    },
    "hmpos": {
        "name": "HmPos (Home)",
        "description": "Second waypoint - Home Position",
        "angles": [777.0, 0.0, 130.0, 0.0]  # Actual values from your DB
    },
    "pos3": {
        "name": "Pos3 (Third)",
        "description": "Third waypoint",
        "angles": [925.0, 192.0, 99.0, 0.0]  # Actual values from your DB
    },
    "pos4": {
        "name": "Pos4 (Fourth)",
        "description": "Fourth waypoint - Stop",
        "angles": [550.0, 500.0, 550.0, 30.0]  # Actual values from your DB
    },
    "go2pos": {
        "name": "Go2Pos (Manual)",
        "description": "Manual target position",
        "angles": [550.0, 440.0, 330.0, 22.0]  # Actual values from your DB
    }
}

# Additional custom points (you can modify these)
CUSTOM_POINTS = {
...
async def run_opcua_client_stable():
    """Stable OPC UA client with proper reconnection handling"""
...
# ============================================================
# Write to PLC Functions (معدلة مثل run_plc.py)
# ============================================================

async def write_go2pos_and_execute(angles):
    """Write to Go2Pos and execute motion - مثل run_plc.py"""
    try:
        async with Client(SERVER_URL, timeout=5) as client:
            ...
            # 4. تشغيل EnableKinematics (توليد النبضة)
            dv = ua.DataValue(ua.Variant(True, ua.VariantType.Boolean))
            dv.SourceTimestamp = None
            dv.ServerTimestamp = None
            await enable_kin_node.write_value(dv)
            
            print(f"   ✅ Go2Pos: {angles}")
            return True, "Success"
    except Exception as e:
        print(f"   ❌ Error: {e}")
        return False, str(e)
...
class DigitalTwinGUI:
    def __init__(self):
        self.root = tk.Tk()
        self.root.title("🤖 Digital Twin V6.1 - Waypoint Control")
    ...
    def create_widgets(self):
        title = tk.Label(self.root, text="Digital Twin V6.1 - Waypoint Control", font=("Arial", 18, "bold"))
        title.pack(pady=10)
